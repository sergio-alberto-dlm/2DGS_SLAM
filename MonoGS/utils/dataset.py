...
def load_dataset(args, path, config):
    ...
    elif config["Dataset"]["type"] == "custom_data":
...
class CustomParser:
    ...
    def load_poses(self, poses_file):
        ...
        with open(poses_file, 'r') as f:
            for line in f:
                # Assuming each line contains a 4x4 matrix flattened in row-major order
                values = list(map(float, line.strip().split()))
                if len(values) != 16:
                    raise ValueError("Each pose must have 16 values representing a 4x4 matrix.")
                pose_matrix = np.array(values, dtype=np.float32).reshape((4, 4))
