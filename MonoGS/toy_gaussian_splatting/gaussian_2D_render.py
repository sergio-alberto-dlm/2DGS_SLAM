...
from toy_utils.utils import getProjectionMatrix, focal2fov
...
def surface_splatting(means3D, scales, quats, viewpoint, projmat, colors, opacities, intrins, device):
    ...
    # last_row = torch.tensor([[0., 0., 0., 1.]], device=viewpoint.R.device, dtype=viewpoint.R.dtype)
    # viewmat = torch.cat([
    #     torch.cat([viewpoint.R, viewpoint.T.unsqueeze(1)], dim=1),
    #     last_row
    # ], dim=0)
    ...
    projmat[-1,-2] = 1.0
    projmat = projmat.T
    T, colors, opacities, center, depth, radii = setup(means3D, scales, quats, viewmat.T, projmat, colors, opacities, device)
    ...
    return {
        "render" : image.permute(2, 0, 1), # C, H, W as the slam format  
        "depth"  : depthmap, 
        "opacity": opacity_map,
        "center" : center, 
        "radii"  : radii, 
